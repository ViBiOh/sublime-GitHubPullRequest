...
- **Keep the core `sublime`-free.** Only `plugin.py` and `githubpullrequest/anchors.py` may `import sublime`. Everything else (`urls`, `diff`, `mapper`, `render`, `gh`, `review`, `state`, `repo`, `owners`, `layout`, `labels`, `panel`, `jsonc`) must not, which keeps it unit-testable headlessly. When a helper needs a view, duck-type it (`repo.rel_path` only calls `view.file_name()`) instead of importing `sublime`.
...
| `jsonc.py` | `loads` / `load_file` for Sublime's JSON-with-comments resource files (settings, menus, keymaps). Strips WHOLE-LINE `//` comments only; a trailing-comment stripper would need to track string literals or it truncates any value containing `//`. Used by the tests that validate the packaged resources. |
...
| `panel.py` | Text model for the changed-files panel: `files_panel_text(open_paths)`, `first_change_line`, `first_comment_line`, `drafts_for_path`. Reads `SESSION`; builds text only (colouring is the syntax file's job). Rows carry a fixed-width marker slot (`● ` when the file is open as a tab) so the path column stays aligned. A file row points at the first ADDED/REMOVED line, never at `hunks[0].new_start`: a hunk opens with up to 3 lines of context, so the raw start is a few lines above anything that changed and disagrees with the line GitHub shows. |
...
- **A renamed file has two paths, and the base side keeps the old one.** Session entries carry both: `path` (head side, what `rel_path(view)` yields and what threads/comments are keyed by) and `base_path` (`FileDiff.old_path`, where the file sat at the merge base). Anything reading base-side content must use `base_path` — `git show <merge_base>:<head path>` simply fails for a moved file, and since `_apply_reference_document` swallows that into `base = None`, the whole file loses its gutter diff with NO error anywhere. Only a file the PR ADDS legitimately has no base blob, and that case is covered by the `is_new` fallback to `""`.
...
- **Naming**: command classes are `GithubPullRequest<Verb>Command`; Sublime derives the command id by snake_casing minus `Command` (e.g. `GithubPullRequestLoadCommand` → `github_pull_request_load`). That `Githu` is deliberate and is the ONE place the package's display name (`GitHubPullRequest`, everywhere a user can read it) must not be followed: Sublime breaks on each capital that follows a lowercase, so `GitHubPullRequestLoadCommand` would become `git_hub_pull_request_load` and silently kill every palette, menu and keymap entry at once. Keep `Default.sublime-commands`, `Main.sublime-menu` and `run_command(...)` strings in sync; `menu_test.py` checks the three resource files against the classes in `plugin.py`, since a stale command string is a silently dead entry.
- **Every user-facing command needs a menu entry, not just a palette one.** Package Control rejects a package that binds keys with no menu (nothing reveals or lets the user rebind them), and Sublime prints a command's binding beside its menu item. `Main.sublime-menu` therefore carries Tools → GitHubPullRequest (all commands, including the compose-only `github_pull_request_submit_comment`) and Preferences → Package Settings → GitHubPullRequest (Settings / Key Bindings via `edit_settings`, README). A command listed in a menu must have a truthful `is_enabled`, or it stays clickable in a context where it does nothing.
- **`${packages}/<folder>/` literals must match the published package name.** They resolve against the folder the package is INSTALLED into, which Package Control names after `repository.json`, not after the git clone. `plugin.py` avoids the literal entirely by reading `PACKAGE` off `__name__` (Sublime names a root-level plugin module `<folder>.plugin`), so only `Main.sublime-menu` hardcodes it and `menu_test.py` pins it to `repository.json`. A wrong literal loads nothing and reports nothing.
...
- New command → add a `GithubPullRequest<Verb>Command` in `plugin.py` (the root module, since Sublime only discovers commands there), an entry in `Default.sublime-commands`, and one in `Main.sublime-menu` (`menu_test.py` fails if the menu is missing it).
