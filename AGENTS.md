...
- **Naming**: command classes are `GithubPullRequest<Verb>Command`; Sublime derives the command id by snake_casing minus `Command` (e.g. `GithubPullRequestLoadCommand` → `github_pull_request_load`). Keep `Default.sublime-commands`, `Main.sublime-menu` and `run_command(...)` strings in sync; `menu_test.py` checks the three resource files against the classes in `plugin.py`, since a stale command string is a silently dead entry.
- **Every user-facing command needs a menu entry, not just a palette one.** Package Control rejects a package that binds keys with no menu (nothing reveals or lets the user rebind them), and Sublime prints a command's binding beside its menu item. `Main.sublime-menu` therefore carries Tools → GitHub Pull Request (all commands, including the compose-only `github_pull_request_submit_comment`) and Preferences → Package Settings → GitHub Pull Request (Settings / Key Bindings via `edit_settings`, README). A command listed in a menu must have a truthful `is_enabled`, or it stays clickable in a context where it does nothing.
