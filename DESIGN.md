...
- `Main.sublime-menu` — the discoverable surface, in two places. **Tools → GitHub Pull Request** repeats every palette command (menu captions are Title Case and unprefixed, the submenu already groups them) plus `github_pull_request_submit_comment`, which is in the palette-less compose buffer and would otherwise have no way of showing its Save binding. **Preferences → Package Settings → GitHub Pull Request** offers Settings, Key Bindings (both via `edit_settings`, which opens the packaged default beside the User override) and the README. Package Control requires this: a package that binds keys but shows no menu cannot be discovered or customized. `${packages}/<folder>/` paths resolve against the INSTALLED folder, so the literal must equal the package name in `repository.json`; `menu_test.py` asserts that, plus that every palette-, menu- and key-bound command actually exists in `plugin.py`.
